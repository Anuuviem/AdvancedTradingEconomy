...
if (Z_SingleCurrency) then {
	{
		_money = (round(_money + ((((_itemsCheckArray select _forEachIndex) select 0)) * _x) * DZE_sellMult));
		_itemsToLog set [2, (_itemsToLog select 2) + [((((_itemsCheckArray select _forEachIndex) select 0)) * _x)]];
	} forEach (_outcome select 0);
	{
		_money = (round(_money + ((((_weaponsCheckArray select _forEachIndex) select 0)) * _x) * DZE_sellMult));
		_itemsToLog set [2, (_itemsToLog select 2) + [((((_weaponsCheckArray select _forEachIndex) select 0)) * _x)]];
	} forEach (_outcome select 1);
	{
		_money = (round(_money + ((((_bpCheckArray select _forEachIndex) select 0)) * _x) * DZE_sellMult));
	...
	if (count _outcome > 3) then {
		_money = (round(_money + ((_vehCheckArray select 0) select 0) * DZE_sellMult));
