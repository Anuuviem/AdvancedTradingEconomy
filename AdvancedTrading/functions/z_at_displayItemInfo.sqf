...
if (Z_Selling) then {
	_buyPrice = (round((_item select 6) * DZE_buyMult));
	_buyCurrency = _item select 8;
	_sellPrice = (round((_item select 2) * DZE_sellMult));
	_sellCurrency = _item select 7;
} else {
	_buyPrice = (round((_item select 2) * DZE_buyMult));
	_buyCurrency = _item select 7;
	_sellPrice = (round((_item select 6) * DZE_sellMult));
