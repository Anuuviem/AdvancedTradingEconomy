...
if (Z_SingleCurrency) then {
	...
	if (Z_Selling) then {
		{
			_sellPrice = (round(_sellPrice + (_x select 2) * DZE_sellMult));
		} count Z_SellArray;
		...
		{
			_sellPrice = (round(_sellPrice +  (((_x select 2) * (_x select 9)) * DZE_buyMult)));
	...
	if (Z_Selling) then {
			_ctrlText = "";
			{
				_sellPrice = (round(_sellPrice +  (((_x select 2) * (_x select 11)) * DZE_sellMult)));
			} count Z_SellArray;
			_ctrlText = [_sellPrice,false] call Z_calcCurrency;
			(findDisplay Z_AT_DIALOGWINDOW displayCtrl Z_AT_RIGHTLISTTITLE) ctrlSetText format ["%1 (%2 items)", localize "STR_EPOCH_TRADE_SELLING", count Z_SellArray];
		} else {
			{
				_sellPrice = (round(_sellPrice +  (((_x select 2) * (_x select 11) * (_x select 9)) * DZE_buyMult)));
				_bTotal = _bTotal + (_x select 9);
			} count Z_BuyingArray;
			_ctrlText = [_sellPrice,false] call Z_calcCurrency;
			(findDisplay Z_AT_DIALOGWINDOW displayCtrl Z_AT_RIGHTLISTTITLE) ctrlSetText format ["%1 (%2 items)", localize "STR_EPOCH_TRADE_BUYING", _bTotal];
